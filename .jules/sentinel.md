...
## 2026-03-30 - [Command Injection via PowerShell Interpolation]
**Vulnerability:** Constructing PowerShell commands by interpolating variables into a script string passed to the `-Command` flag (e.g. `$"-Command \"{script}\""`) allows command injection if the variables or script text can be manipulated, or if escape sequences are improperly handled by the shell.
**Learning:** Using `-Command` with string interpolation for dynamically generated scripts is fundamentally unsafe because PowerShell interprets the unescaped string as code. Attempts to manually escape and wrap scripts in single quotes via `ArgumentList` can also cause functional regressions (PowerShell echoes the string instead of executing it).
**Prevention:** Always safely pass scripts to PowerShell by converting them to a Base64 Unicode string (`Convert.ToBase64String(System.Text.Encoding.Unicode.GetBytes(script))`) and executing them using the `-EncodedCommand` flag. This completely prevents command injection and syntax parsing issues.
