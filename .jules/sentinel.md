...
## 2026-04-03 - [Command Injection via ProcessStartInfo arguments]
**Vulnerability:** A vulnerability was present when calling PowerShell via `ProcessStartInfo`, where the `script` string was passed via string interpolation to the `-Command` argument without encoding, allowing for potential command injection if the script contained unescaped quotes or newlines.
**Learning:** Passing user-supplied or dynamically generated script strings directly to `powershell.exe -Command` using string interpolation within `ProcessStartInfo.Arguments` is unsafe and can lead to command injection.
**Prevention:** Always use `-EncodedCommand` and Base64-encode the Unicode representation of the script before passing it to PowerShell.
