...
## 2026-03-19 - [Command Injection via PowerShell ProcessStartInfo Arguments]
**Vulnerability:** Constructing a PowerShell command string and executing it via `ProcessStartInfo.Arguments` with string interpolation (e.g., `-Command "{script}"`) allows arbitrary command injection. Even if arguments are quoted or using `ArgumentList` with `-Command`, PowerShell may evaluate subexpressions (`$()`) within the string unless rigorously escaped.
**Learning:** Using `-Command` for complex scripts executed from C# is inherently fragile and prone to injection attacks when script contents change or combine with external variables. Using `ArgumentList` doesn't inherently prevent PowerShell from evaluating subexpressions if `-Command` is used.
**Prevention:** To safely execute dynamic PowerShell scripts from C# without risk of injection or quoting bugs, convert the script to a Base64-encoded Unicode string and pass it using the `-EncodedCommand` flag.
