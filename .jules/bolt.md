...
## 2026-04-04 - ManagementObject vs ManagementObjectSearcher for exact keys
**Learning:** When querying WMI for a single, specific instance by its primary key (like `DeviceID` for a PnP Device), using `ManagementObjectSearcher` with a WQL `WHERE` clause forces WMI to parse the query and evaluate it.
**Action:** For exact key lookups, use direct object instantiation via `new ManagementObject("ClassName.KeyProperty=\"Value\"")`. This entirely bypasses WQL parsing and evaluation overhead. Remember to catch `ManagementException` when calling `.Get()` as absent devices will throw instead of returning an empty collection.
