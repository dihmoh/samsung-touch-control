...
## 2024-05-19 - Fast WMI single-device lookups
**Learning:** For maximum WMI performance when querying a single device by its primary key (e.g., DeviceID), using ManagementObjectSearcher with a WQL WHERE clause introduces parsing and query evaluator overhead. Direct instantiation via ManagementObject (e.g. `new ManagementObject($"Win32_PnPEntity.DeviceID=\"{queryId}\"")`) completely bypasses this and is significantly faster.
**Action:** When the primary key of a WMI object is known, instantiate `ManagementObject` directly instead of querying with `ManagementObjectSearcher`.
