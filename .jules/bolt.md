...
## 2025-01-28 - WMI Primary Key Lookups via Direct Instantiation
**Learning:** Using `ManagementObjectSearcher` with a WQL `WHERE` clause (e.g., `SELECT Status FROM Win32_PnPEntity WHERE DeviceID = '...'`) incurs unnecessary WQL parsing and WMI query evaluator overhead, even for single device lookups by primary key.
**Action:** For maximum WMI performance when querying a single device by its primary key (e.g., `DeviceID`), instantiate a `ManagementObject` directly using the exact WMI path (e.g., `new ManagementObject("Win32_PnPEntity.DeviceID=\"...\"")`). Wrap `.Get()` or method invocations in a `try/catch` for `ManagementException` to handle cases where the device is not found safely.
