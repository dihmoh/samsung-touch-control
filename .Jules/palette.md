...
## 2026-03-22 - WebView2 HTML Embedded UI Accessibility
**Learning:** Even when UI components are embedded within a C# Windows Forms application via a WebView2 HTML template, standard web accessibility rules apply. Interactive elements like `div` acting as buttons (e.g., hotkey settings or startup toggle) and icon-only buttons (`✕`, large `ON/OFF` graphics) lack semantics for screen readers and default focus styles for keyboard users, making them inaccessible.
**Action:** When implementing embedded HTML UIs, explicitly add `aria-label` or `title` attributes to icon-only buttons using localized strings (e.g., `{Strings.AccessibleClose}`). For non-button interactive elements like `div`, add `role='button'`, `tabindex='0'`, and corresponding keyboard event handlers (like `onkeydown`). Finally, always implement `:focus-visible` pseudo-classes to ensure keyboard navigation provides clear visual feedback.
