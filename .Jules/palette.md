...
## 2024-05-18 - WebView2 Keyboard Accessibility and ARIA Attributes
**Learning:** When embedding an HTML UI via WebView2 using C# interpolated verbatim strings (`$@"..."`), standard web accessibility guidelines must be applied directly to the HTML. This includes defining `aria-label` or `title` for icon-only buttons and assigning `role='button'`, `tabindex='0'`, and `onkeydown` handlers to interactive `div`s. Additionally, any double quotes within the HTML attributes must be double-escaped as `""` to prevent C# compilation errors.
**Action:** Apply `:focus-visible` styles for keyboard navigation and ensure `event.preventDefault()` is called in `onkeydown` handlers to stop unwanted browser scrolling.
