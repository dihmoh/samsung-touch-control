...
#pragma warning disable CA1416
                ...
                // ⚡ Bolt: Direct WMI instantiation by primary key (DeviceID) bypasses WQL parsing
                // and query evaluation overhead, making this check nearly instantaneous.
                try
                {
                ...
                // Fallback to null if device is physically disconnected but still cached.
                // Searcher.Get() used to just return empty, so we return null to avoid the slow PowerShell fallback.
        ...
        public bool SetTouchState(bool enable, ConfigManager config)
        {
            try
            {
                ...
                try
                {
                    string queryId = id.Replace("\\", "\\\\");
#pragma warning disable CA1416
                    // ⚡ Bolt: Direct WMI instantiation by primary key
                    try
                    {
                        using var device = new System.Management.ManagementObject($"Win32_PnPEntity.DeviceID=\"{queryId}\"");
                        device.Get();
