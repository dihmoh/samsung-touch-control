...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; user-select: none; }}
  ...
  .btn-close:focus-visible {{ outline: 2px solid white; outline-offset: 2px; }}
  ...
  .btn-toggle:focus-visible {{ outline: 2px solid white; outline-offset: 2px; }}
  ...
  .hotkey-container:focus-visible {{ outline: 2px solid white; outline-offset: 2px; }}
  ...
  .card-startup:focus-visible {{ outline: 2px solid white; outline-offset: 2px; }}
...
<div class='header'>
  ...
  <button class='btn-close' onclick=""window.chrome.webview.postMessage('close')"" title='{Strings.AccessibleClose}' aria-label='{Strings.AccessibleClose}'>✕</button>
</div>

<div class='content-area'>
  <div class='card card-main'>
    <div class='ring-container'>
      <button class='btn-toggle' onclick=""window.chrome.webview.postMessage('toggle')"" title='{Strings.AccessibleToggle}' aria-label='{Strings.AccessibleToggle}'>
        {touchIcon}
      </button>
    </div>
    <div class='status'>{statusText}</div>
    <div class='hotkey-container' onclick=""window.chrome.webview.postMessage('change_hotkey')"" role='button' tabindex='0' onkeydown=""if(event.key==='Enter'||event.key===' ') {{ event.preventDefault(); this.click(); }}"">
      ⌨ {hotkeyText} ✏️
    </div>
  </div>

  <div class='card card-startup' onclick=""window.chrome.webview.postMessage('startup')"" role='switch' aria-checked='{_config.GetStartWithWindows().ToString().ToLower()}' tabindex='0' onkeydown=""if(event.key==='Enter'||event.key===' ') {{ event.preventDefault(); this.click(); }}"">
    <div class='text-col'>
