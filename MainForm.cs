...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; user-select: none; }}
  ...
  .btn-toggle {{
    ...
    display: flex; align-items: center; justify-content: center;
    transition: background 0.3s, transform 0.1s, outline 0.15s;
  }}
  .btn-toggle:hover {{ filter: brightness(1.1); transform: scale(1.02); }}
  .btn-toggle:active {{ transform: scale(0.95); }}
  .btn-toggle:focus-visible { outline: 3px solid {statusColor}; outline-offset: 4px; }
  ...
  .hotkey-container:focus-visible { outline: 2px solid rgba(255,255,255,0.5); outline-offset: 2px; background: rgba(255,255,255,0.12); }
  ...
  .card-startup:focus-visible { outline: 2px solid {accentColor}; outline-offset: 2px; background: #2c2c4d; }
...
<div class='content-area'>
  <div class='card card-main'>
    ...
    <div class='hotkey-container' role='button' tabindex='0' aria-label='{Strings.BalloonHotkey(_config.HotkeyModifier, _config.HotkeyKey)}' onclick=""window.chrome.webview.postMessage('change_hotkey')"" onkeydown=""if(event.key === 'Enter' || event.key === ' ') {{ event.preventDefault(); window.chrome.webview.postMessage('change_hotkey'); }}"">
      ⌨ {hotkeyText} ✏️
    </div>
  </div>

  <div class='card card-startup' role='switch' tabindex='0' aria-checked='{_config.GetStartWithWindows().ToString().ToLower()}' onclick=""window.chrome.webview.postMessage('startup')"" onkeydown=""if(event.key === 'Enter' || event.key === ' ') {{ event.preventDefault(); window.chrome.webview.postMessage('startup'); }}"">
    <div class='text-col'>
