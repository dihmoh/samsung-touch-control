...
<div class='content-area'>
  <div class='card card-main'>
    <div class='ring-container'>
      <button class='btn-toggle' aria-label=""{Strings.AccessibleToggle} - {statusText}"" onclick=""window.chrome.webview.postMessage('toggle')"">
        {touchIcon}
      </button>
    </div>
    <div class='status'>{statusText}</div>
    <div class='hotkey-container' role=""button"" tabindex=""0"" onkeydown=""if(event.key==='Enter'||event.key===' ') {{ event.preventDefault(); this.click(); }}"" onclick=""window.chrome.webview.postMessage('change_hotkey')"">
      ⌨ {hotkeyText} ✏️
    </div>
  </div>

  <div class='card card-startup' role=""switch"" aria-checked=""{(_config.GetStartWithWindows() ? "true" : "false")}"" tabindex=""0"" onkeydown=""if(event.key==='Enter'||event.key===' ') {{ event.preventDefault(); this.click(); }}"" onclick=""window.chrome.webview.postMessage('startup')"">
    <div class='text-col'>
