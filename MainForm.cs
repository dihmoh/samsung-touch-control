...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; user-select: none; }}
  ...
  .btn-toggle:focus-visible {{ outline: 3px solid rgba(255,255,255,0.7); outline-offset: 4px; }}
...
<div class='content-area'>
  <div class='card card-main'>
    ...
    <div class='hotkey-container' role='button' tabindex='0' aria-label='{hotkeyText}' onclick=""window.chrome.webview.postMessage('change_hotkey')"" onkeydown=""if(event.key === 'Enter' || event.key === ' ') {{ event.preventDefault(); window.chrome.webview.postMessage('change_hotkey'); }}"">
      ⌨ {hotkeyText} ✏️
    </div>
  </div>

  <div class='card card-startup' role='switch' aria-checked='{_config.GetStartWithWindows().ToString().ToLower()}' tabindex='0' onclick=""window.chrome.webview.postMessage('startup')"" onkeydown=""if(event.key === 'Enter' || event.key === ' ') {{ event.preventDefault(); window.chrome.webview.postMessage('startup'); }}"">
    <div class='text-col'>
